...
For normal rehearsals, use `make acceptance-gate` instead of manually running the five commands above. The individual commands remain useful for isolating a failed or blocked stage.
...
- Local acceptance-gate tests prove `run_acceptance_gate` writes all gate artifacts into a run-scoped directory, executes production readiness exactly once, reuses that same readiness result inside business-trial acceptance, overwrites stale artifacts from prior runs, reaches a fully green path with injected PASS evidence, runs safely in an empty env as `BLOCKED`, and redacts secret-valued env vars, Bearer, `sk-*`, common DSN passwords, and WeCom webhook values across written JSON/Markdown artifacts.
