...
Remaining before business trial: run `business_trial_acceptance_bundle` with real production URLs, production ycapi token policy, a live WeCom webhook plus alert report, real ycapi monthly bill evidence, AC-23 human trial evidence, and AC-26 HR/legal acknowledgment evidence until every check in `/tmp/aimanager-business-trial-acceptance.json` is `PASS`.
