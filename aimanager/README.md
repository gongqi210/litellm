...
For normal rehearsals, use `make acceptance-gate` instead of manually running the commands above; it also writes the evidence handoff and template pack into the same output directory, while `make evidence-intake` validates owner-filled evidence before reruns. The individual commands remain useful for isolating a failed or blocked stage.
