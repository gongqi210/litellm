...
上表中的 `BLOCKED` 项不是失败，而是完整 AC 还缺 RBAC、部分错误契约/故障场景、生产管理面暴露边界或真实 live ycapi 证据。M1 试点前必须解除这些 `BLOCKED` 项；生产告警路由需在部署侧把 management metrics 或 observability JSON report 接入既定告警渠道。
