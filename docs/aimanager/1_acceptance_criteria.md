...
| AC-06 | BLOCKED | `PYTHONPATH="$PWD" uv run --no-project --with pytest pytest aimanager/tests/test_sdk_compat_smoke.py aimanager/tests/test_mock_ycapi.py -q`；`env -u AIMANAGER_EMPLOYEE_VIRTUAL_KEY PYTHONPATH="$PWD" uv run --no-project python -m aimanager.scripts.smoke_sdk_compat --base-url http://localhost:4000` | 本地合同已覆盖 employee virtual key 调用边界、误用 `YCAPI_API_TOKEN` 变量/同值密钥 fail-closed、`/v1/models`、`gemini-2.5-flash`/`deepseek-chat` chat、`ycapi-image-1` image `response_format=b64_json`、base64 `data:` URL vision、必填工作 metadata、OpenAI-compatible 响应形态和失败脱敏；README 已提供 curl/Python/Node 示例，且 smoke 不读取/打印 `YCAPI_API_TOKEN`。当前环境缺真实员工 virtual key 且 business proxy 未提供可验证运行态，因此 AC-06 运行态保持 `BLOCKED`，待部署侧设置 `AIMANAGER_EMPLOYEE_VIRTUAL_KEY` 并对业务 URL 跑出 `PASS` 后解除 |
...
上表中的 `BLOCKED` 项不是失败，而是完整 AC 还缺真实员工 virtual key 运行态、生产管理面暴露边界或真实 live ycapi 证据。M1 试点前必须解除这些 `BLOCKED` 项；生产告警路由已有 WeCom 脚本化入口，但仍需用真实 webhook 跑出 live routing 证据。
