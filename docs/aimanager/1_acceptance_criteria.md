...
| AC-17 | BLOCKED | `PYTHONPATH="$PWD" uv run --no-project --with pytest --with pyyaml pytest aimanager/tests/test_policy.py -q`；`aimanager.litellm_entrypoint` 强制本地 model cost map | policy 层证明不会切 provider passthrough；启动期外网/TLS 异常不会阻塞 LiteLLM cost map 远程拉取；mock 下游 ycapi/LiteLLM 429 和 5xx 已验证为可解释 OpenAI-compatible error 且不泄密；Postgres down 仍未跑 |
...
上表中的 `BLOCKED` 项不是失败，而是完整 AC 还缺 RBAC、Postgres down 故障场景、生产管理面暴露边界或真实 live ycapi 证据。M1 试点前必须解除这些 `BLOCKED` 项；生产告警路由需在部署侧把 management metrics 或 observability JSON report 接入既定告警渠道。
