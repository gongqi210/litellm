...
| AC-06 | BLOCKED | `PYTHONPATH="$PWD" uv run --no-project --with pytest pytest aimanager/tests/test_sdk_compat_smoke.py aimanager/tests/test_runtime_sdk_compat_smoke.py aimanager/tests/test_mock_ycapi.py -q`；`env -u AIMANAGER_EMPLOYEE_VIRTUAL_KEY PYTHONPATH="$PWD" uv run --no-project python -m aimanager.scripts.smoke_sdk_compat --base-url http://localhost:4000`；`LITELLM_MASTER_KEY=<master> PYTHONPATH="$PWD" uv run --no-project python -m aimanager.scripts.smoke_runtime_sdk_compat --master-key <master> --business-base-url <business-url> --admin-base-url <admin-url>` | 本地合同已覆盖 employee virtual key 调用边界、误用 `YCAPI_API_TOKEN` 变量/同值密钥 fail-closed、`/v1/models`、`gemini-2.5-flash`/`deepseek-chat` chat、`ycapi-image-1` image `response_format=b64_json`、base64 `data:` URL vision、必填工作 metadata、OpenAI-compatible 响应形态和失败脱敏；README 已提供 curl/Python/Node 示例，且 smoke 不读取/打印 `YCAPI_API_TOKEN`。新增 `smoke_runtime_sdk_compat` 可通过管理面创建一次性治理 employee LiteLLM virtual key、经业务面运行 SDK smoke、并带 lifecycle audit headers 删除 disposable key；清理失败即 `FAIL`，避免遗留员工 key。当前环境未运行可验证 business/admin surface，因此 AC-06 运行态保持 `BLOCKED`，待部署侧对真实业务/管理 URL 跑出 `PASS` 后解除 |
...
上表中的 `BLOCKED` 项不是失败，而是完整 AC 还缺 disposable-key SDK runtime smoke、生产管理面暴露边界或真实 live ycapi 证据。M1 试点前必须解除这些 `BLOCKED` 项；生产告警路由已有 WeCom 脚本化入口，但仍需用真实 webhook 跑出 live routing 证据。
