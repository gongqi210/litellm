...
上表中的 `BLOCKED` 项不是失败，而是完整 AC 还缺生产管理面暴露边界或真实 live ycapi 证据。M1 试点前必须运行 `production_readiness_bundle` 并解除 JSON 中所有 `BLOCKED` 项；当前空环境验证结果为 exit code `2`，4 项 `BLOCKED`：AC-15 生产 URL、AC-19 生产 ycapi token、AC-16 WeCom webhook/report、AC-12/13 真实 spend 与 ycapi 月账单文件。财务输入为空或两侧对账没有非零计费金额时同样保持 `BLOCKED`，不能用空 CSV 表头或占位行作为月结证据。
