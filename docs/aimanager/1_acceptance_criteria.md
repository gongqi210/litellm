...
| AC-10 | PASS | `python -m aimanager.scripts.smoke_budget_block` + LiteLLM container log | mock ycapi + rebuilt business/admin proxy 已验证：一次性治理 key `max_budget=0.005` 先经 image 成功消费 `0.01`，随后同 key 的 business chat 请求返回 HTTP 429，`error.type=budget_exceeded`，响应和容器日志均记录 `Budget has been exceeded! ... Current cost: 0.01, Max budget: 0.005`；`LiteLLM_VerificationToken.spend` 可因批量写延迟仍为 `0.0`，不作为阻断主证据 |
...
| AC-16 | BLOCKED | `pytest aimanager/tests/test_audit.py`；`pytest aimanager/tests/test_policy.py`；运行中 business proxy log：34 条 `aimanager_audit_event`，admin proxy log：`/config/field/update` policy event；运行中 key governance smoke 日志：`request_id=runtime-key-governance-smoke` 对应 `policy_blocked`，secret-like 检查无 Authorization/Bearer/本地占位 token；`python -m aimanager.scripts.smoke_budget_block` 触发 LiteLLM 429 budget log；`LITELLM_LOCAL_MODEL_COST_MAP=True` 入口保护已验证 | passthrough/config/cache/reload/model-write/default-denied policy block 已有运行时结构化审计日志；key 创建治理拒绝也会记录 `policy_blocked` 审计事件且不记录请求密钥；预算阻断原因已在 429 响应和 LiteLLM 日志中可观测，但 AiManager 结构化 `budget_blocked` emitter、key 冻结/撤销、metrics/告警仍需接入 |
...
上表中的 `BLOCKED` 项不是失败，而是完整 AC 还缺 RBAC、部分错误契约/故障场景、AiManager 结构化预算与 key 生命周期审计 emitters、导出任务或真实 ycapi 账单证据。M1 试点前必须解除这些 `BLOCKED` 项。
