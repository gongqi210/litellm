...
| AC-16 | PASS | `pytest aimanager/tests/test_audit.py aimanager/tests/test_policy.py aimanager/tests/test_observability.py aimanager/tests/test_observability_export_script.py aimanager/tests/test_observability_alert_route_script.py`；运行中 business proxy log：policy block `aimanager_audit_event`；admin proxy log：`/config/field/update` policy event；运行中 key governance smoke 日志：`request_id=runtime-key-governance-smoke` 对应 `policy_blocked`，secret-like 检查无 Authorization/Bearer/本地占位 token；`python -m aimanager.scripts.smoke_budget_block` 触发 LiteLLM 429 budget log 和 AiManager `budget_blocked` 结构化审计事件；`python -m aimanager.scripts.smoke_key_lifecycle` 触发 `key_frozen`/`key_revoked` 结构化审计事件；`PYTHONPATH="$PWD" uv run --no-project python -m aimanager.scripts.export_observability --help`；`PYTHONPATH="$PWD" uv run --no-project python -m aimanager.scripts.route_observability_alerts --report-file <report.json> --dry-run --output-payload-file <payload.json>`；`LITELLM_LOCAL_MODEL_COST_MAP=True` 入口保护已验证 | passthrough/config/cache/reload/model-write/default-denied policy block 已有运行时结构化审计日志；key 创建治理拒绝会记录 `policy_blocked` 且不记录请求密钥；预算阻断已有 429 响应、LiteLLM 日志和 AiManager `budget_blocked` 结构化审计事件；key 冻结/撤销已有管理面 lifecycle 审计 emitters；management-only `GET /metrics` 由 AiManager 直接输出低基数 `aimanager_audit_events_total` 与 `aimanager_http_responses_total`，business `/metrics` 仍按 policy 阻断；`export_observability` 可从审计日志和 request-status JSON/CSV 生成 failure rate、429/5xx、预算阻断、passthrough 拦截、缺 request id 等机器可读 alert 记录；`route_observability_alerts` 可把 alert JSON 渲染并投递到企业微信 webhook，支持 dry-run、severity 过滤、缺 webhook 返回 BLOCKED、WeCom `errcode=0` 验证且不打印 webhook URL；真实生产 webhook 投递仍需在部署侧用 `AIMANAGER_WECOM_WEBHOOK_URL` 跑一次 live routing |
...
| AC-19 | BLOCKED | `pytest aimanager/tests/test_live_ycapi_smoke.py`；`env -u YCAPI_API_TOKEN PYTHONPATH="$PWD" uv run --no-project python -m aimanager.scripts.smoke_live_ycapi --expect-model gemini-2.5-flash --expect-model deepseek-chat --expect-model ycapi-image-1` | `smoke_live_ycapi` 是 live ycapi token preflight：缺真实 `YCAPI_API_TOKEN` 时返回 exit 2 和 `BLOCKED`，不会误报 `PASS`；有 token 时只读调用 ycapi `/models` 并校验期望模型，不打印 token、请求 URL 或响应 body。当前环境未提供真实生产 ycapi token，因此该项保持 `BLOCKED`，待部署侧用生产 token 跑出 `PASS` 后解除 |

上表中的 `BLOCKED` 项不是失败，而是完整 AC 还缺生产管理面暴露边界或真实 live ycapi 证据。M1 试点前必须解除这些 `BLOCKED` 项；生产告警路由已有 WeCom 脚本化入口，但仍需用真实 webhook 跑出 live routing 证据。
